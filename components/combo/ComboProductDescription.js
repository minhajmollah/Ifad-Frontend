...
import {hasComboReviewAbility, saveReview, fetchComboReviews} from "../../services/ReviewServices";
...
const ComboProductDescription = ({combo}) => {
	...
	const hasComboReviewAbilityStatus = (comboId) => {
		hasComboReviewAbility(comboId).then((response) => {
			if (response?.data) {
	...
	useEffect(() => {
		if (combo?.id) {
			hasComboReviewAbilityStatus(combo.id);
		}
	}, [combo?.id])

	useEffect(() => {
		if (combo?.id) {
			fetchComboReviews(combo.id).then(response => {
								...
								<div className="d-flex justify-content-start w-100">
									...
									<div className="">
										...
										<div className="d-flex justify-content-start align-items-center">
											<StarRatings
												rating={parseInt(combo?.star_ratting || 0)}
